...
data %>% head() %>%kable(format = "pipe", digits = 2) 
```
...
For Random Effects, I have selected Lecturer and Group-Lecturer interaction:
...
Groups are of direct interest (fixed) and Lecturers are a sample from a larger population of potential assessors (random). Previous marks are objective measures that should influence current marks (fixed).
...
pred_intervals <- posterior_predict(model, newdata = newdata, re_formula = NA) %>% 
  ...
  mutate(Group = newdata$Group)


final_table <- left_join(group_estimates, pred_intervals, by = "Group") %>%
  select(Group, estimate, lower.HPD, upper.HPD, pred_lower, pred_upper)

final_table %>% 
  kable(digits = 2, 
        col.names = c("Group", "Estimate", "Lower CI", "Upper CI", "Pred Lower", "Pred Upper"),
        caption = "Group Estimates and Prediction Intervals")
...
```{r,warning=FALSE}
cat("GitHub Repository: [Mixed Effects Analysis](https://github.com/enocksoko/MixedEffects)")
```